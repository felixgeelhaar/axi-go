...
- [`example/mcp-server/`](example/mcp-server/) — an MCP (Model Context Protocol) adapter sketch in ~250 lines, no external deps. **Copy/vendor only — not a supported axi-go package; first-party MCP in core is declined.**
...
- [`example/saga/`](example/saga/) — sagas-as-plugin reference: in-process outbox + `OrchestratorActionExecutor` with **fail-closed** nested `write-external` approval. Durable log stays outside axi-go core.
